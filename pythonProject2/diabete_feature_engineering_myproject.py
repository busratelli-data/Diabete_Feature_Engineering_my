...
def grab_col_names(dataframe, cat_th=10, car_th=20):
    ...
    cat_cols = [col for col in cat_cols if
                col not in cat_but_car]  # cat_cols tüm object  veri tipini tuttugu için içerisinde cat_but_car bulunabilir.
...
def cat_summary(dataframe, col_name, plot=True):
    ...
    if plot:
        ...
        plt.show(block=True)  # Grafik penceresi kapanmayacak
...
for col in cat_cols:
    ...
    def num_summary(dataframe, numerical_col, plot=False):  # plot:true olursa if çalışır.
        quantiles = [0.05, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 0.95,
                     0.99]  # hangi ceyreklikleri istiyorum?
...
# Aykırı Değer Analizi ve Baskılama İşlemi
    ...
    df.loc[
        ((df["Glucose"] >= 100) & (df["Glucose"] <= 125)) & (df["Age"] >= 50), "NEW_AGE_GLUCOSE_NOM"] = "hiddensenior"
